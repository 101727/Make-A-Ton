...
import penguinSfx from '../SFX/penguin.mp3';
...
export function useGameAudio() {
  ...
  useEffect(() => {
    const audio = new Audio(penguinSfx);
    audio.preload = 'auto';
    penguinRef.current = audio;
  ...
  const playPenguin = useCallback(() => {
    if (!penguinRef.current) {
      return;
    }

    penguinRef.current.currentTime = 0;
    penguinRef.current.play().catch(() => {});
  }, []);
